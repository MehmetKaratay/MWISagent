...
def get_regions_for_countries(
    ...
    """
    Parses mwis-regions.csv to map a list of countries to their corresponding region codes.

    Args:
        country_names: A list of country names (e.g., ["Scotland", "Wales"]).
        csv_path: Path to the mwis-regions.csv file.

    Returns:
        A deduplicated list of region codes matching the countries.

    Raises:
        ValueError: If the total number of combined regions exceeds 5.
    """
    target_countries = {name.strip().lower() for name in country_names}
    matched_regions = set()
    ...
    try:
        with open(csv_path, encoding="utf-8") as f:  # nosemgrep: detect-path-traversal
            ...
            for row in reader:
                ...
                if country in target_countries and code:
                    matched_regions.add(code)
    except FileNotFoundError:
        pass  # If the CSV doesn't exist, we return empty or let it be empty

    # Convert set to sorted list for determinism (though tests just check membership)
